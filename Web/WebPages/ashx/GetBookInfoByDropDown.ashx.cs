...
{
    /// <summary>
    /// GetBookInfoByDropDown 的摘要说明
    /// </summary>
    public class GetBookInfoByDropDown : IHttpHandler
    {
        GetIndexBookInfoManager gibim = new GetIndexBookInfoManager();
        public void ProcessRequest(HttpContext context)
        {
            context.Response.ContentType = "text/plain";
            int id = Convert.ToInt32(context.Request["categoryid"]);
            context.Response.Write(GetTableRow(id));
        }

        public string GetTableRow(int categroyid)
        {
            StringBuilder sbHtml = new StringBuilder();
            List<Books> list = gibim.GetBookInfoByDropDown(categroyid);
